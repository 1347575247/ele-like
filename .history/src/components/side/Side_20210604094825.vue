	...
	<div class="side">
		...
		<div class="side_main">
			<vHead class="main_header" title="选择收获地址"  @back="$emit('sback')">
				<span class="add" @click="$router.push('/mine/address/add')">新增地址</span>
			</vHead>
			<!-- 地址选择 -->
      ...
      <div class="recv-addr">
		...
		<div class="side_city" v-show="show.city">
			...
			<div class="search_box" v-show="searchStr">
				<ul>
					  <li class="add_list" v-for="(item,i) in citySearch"  @click="selectCity(item)" :key="i">
						{{item.name}}
					  </li>
				</ul>
			</div>
			   ...
			   <!-- 当前城市 -->
				...
				<van-index-bar>
				 <ul v-for="(items,k,i) in cityName" :key="i">
					  <van-index-anchor :index="k" :sticky="false"/>
					  
					  <li class="add_list" v-for="(item,j) in items" @click="selectCity(item)" :key="j">
					    {{item.name}}
					  </li>
				 </ul>
				</van-index-bar>
...
<script>
	...
	export default{
		...
		setup(){
		const {cityList,cityName,searchStr,citySearch,selectCity,currentCity,show, addrInfo} =cityModel();
