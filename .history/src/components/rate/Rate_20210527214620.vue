    ...
    <div class="rate p-spacing">
      ...
      <div class="rate-delivery">
        ...
        <div class="rate-1">
          ...
          <ul class="rate-tags" v-if="rate.delivery.activeIndex" ref="tag">
            <li v-for="(item, i) in rate.delivery.comments" :key="i" @click="addComment">{{item}}</li>
            <li class="write">
...
<script>
  ...
  import rateModel from './js/rateModel'
    ...
    setup(props, ctx) {
      const {rate, rateObj, overallChange, packageChange, qualityChange, deliverRate, addComment} = rateModel()
      return {
        rate,
        rateObj,
        overallChange,
