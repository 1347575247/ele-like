    ...
    <div class="shoppingCar flex">
      <div class="car_main flex">
        <div class="car_icon"
          ...
          @click="show =!show"
        >
          <b class="icon_count">{{total}}</b> <i class="iconfont icon-gouwuche1"></i>
        </div>
        <div class="car_price">
          <div class="price_top">
            <p v-if="totalPrice.price >= basePrice">
              ...
              <!-- total.price_origin -->
              <span class="origin_price symbol">{{totalPrice.origin_price}}</span>
            </p>
      ...
      <div class="car_submit">
        <div class="submit_base" v-if="basePrice>=totalPrice.price">还差{{(basePrice-totalPrice.price).toFixed(1)}}元起送</div>
    ...
    <!-- 购物车列表 -->
		<transition name="list">
		    <div class="shoppingCar_list" v-show="show" v-if="total>=0">
		        <p class="list_discountsMess">已减{{(totalPrice.origin_price - totalPrice.price).toFixed(1)}}元</p>
                      ...
                      {{food}}
		                    ...
		                    <div class="main_price">
							   ...
							   <basebutton
								...
								@changNum="changeNum({item:food.item,cid:food.cid},$event)"></basebutton>
		                    </div> -->
		                </li>
		            </ul>
		        </div>
...
<script>
  ...
  export default {
    ...
    setup(props, ctx) {
      const {foodState, state: foods, total, cartOk, animateName, rst, totalPrice, basePrice, show} = cartModel(props)
