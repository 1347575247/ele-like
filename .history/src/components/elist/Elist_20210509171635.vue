<template>
  <div class="good-list" v-if="elistData">
    <div class="good-item" v-for="(item, i) in elistData.data" :key="i">
      <div class="img-box">
        <img
  ...
  setup(props, context) {
    ...
    const scrollFn = (e) => {
      ...
      if(data.st + data.ct === _html.scrollHeight) {
        console.log("我滑到底部了")
        console.log(props.elistData.data.length, props.elistData.total)
        content.emit("getNewElistData", {offset: props.elistData.data.length, total:props.elistData.total})
      }
    }
