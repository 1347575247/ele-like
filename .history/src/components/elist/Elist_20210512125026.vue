<template>
  <div class="good-list" v-if="itemData">
    <div class="good-item" v-for="(item, i) in itemData.data" :key="i" :style="showActive(i)" @click="$router.replace('/detail')">
      <div class="img-box">
        <img
      ...
      <div class="desc">
        ...
        <div class="insurrance-box">
          <div class="insure-item" v-for="(acItem, ac) in item.restaurant.activities" :key="ac">
            <span class="icon" :style="{backgroundColor:'#'+acItem.icon_color}">{{acItem.icon_name}}</span>
            <span class="sm-font t_ellipsis"
              >{{acItem.description}}</span
            >

            <div class="activity-num" v-if="ac === 0" @click="showActivityFn(i)">
...
export default {
  ...
  setup(props, context) {
    const {getNewElistData, itemData, calDistance, showActivityFn, showActive, $router} = elistModel()
    return {
      calDistance,
      itemData,
      showActive,
