    ...
    <div class="good-item" v-for="(item, i) in elistData" :key="i">
      ...
      <div class="desc">
        ...
        <div class="insurrance-box">
          <div class="insure-item" v-for="(activity, o) in item.restaurant.supports" :key="o">
            <span class="icon">{{activity.id==7?'保':activity.id==4?'票':''}}</span>
            <span class="sm-font t_ellipsis">{{activity.description}}</span>
            <div class="activity-num" v-if="o === 0">
              <span>{{item.restaurant.supports.length? item.restaurant.supports.length : '0'}}个活动</span>
