      ...
      <div class="main p-spacing">
        <div class="card">
          <ul class="addr-list" v-if="addressList">
            <li class="addr-item" :class="item.tag ? item.tag : ''" v-for="(item,i) in addressList" :key="i" @click="selected(item)">
              <p class="address">{{item.baseAddr}}</p>
              <p class="detail">{{item.detailAddr}}</p>
              <p class="info">{{item.name}}({{item.gender}}) {{item.mobile}}</p>
              <span class="iconfont icon-icon-bainji" @click.stop="toAddressEdit(item)"></span>
            </li>
          </ul>
...
<script>
  ...
  import { Popup, Overlay, Dialog, Toast, Icon } from 'vant'
  export default {
    components: {
      vHead,
      [Popup.name]: Popup,
    ...
    setup() {
      const {showDialog, addressList, toAddressEdit, selected} = addrSModel()
      return {
        showDialog,
        addressList,
        toAddressEdit,
