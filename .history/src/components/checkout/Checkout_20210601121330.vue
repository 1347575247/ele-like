<template>
    ...
    <div class="main">
      ...
      <div class="detail br-radius p-spacing">
        ...
        <ul class="food-list" v-if="foodState" :style="{height: show.isUnfolded ? 'auto' : '9.2rem'}">
        ...
        <ul class="desc" v-if="foodState">
          <li class="item clearFixed">
            <span class="name">{{feeFormat(firstItem(foodState).item.deliver_fee).name}}</span>
...
<script>
  ...
  import { Popup, Overlay, Step, Steps, NoticeBar, Icon, TreeSelect, Sidebar,SidebarItem   } from 'vant';
    ...
    setup(props, ctx) {
      const {show, back, activeId, activeIndex, timeSelectItems, timeChosen, deliver, foodState, firstItem, feeFormat} = checkModel()
