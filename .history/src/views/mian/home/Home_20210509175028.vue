<template>
			  ...
			  <van-swipe-item v-for="(items,i) in spliceFood" :key="i">
				...
				<!-- 分类入口 -->
					<ul class="flex foodType_main" >
						<li class="foodType_item" v-for="(item, j) in items" :key="j">
    ...
    <!-- 首页列表 -->
    <Elist class="p-spacing" :elistData="vlist" @getNewElistData="getNewElistData"></Elist>
  ...
  import Elist from '@comps/elist/Elist.vue'
	...
	export default{
      ...
      Elist,
		...
		setup(){
      const {show,saveSide,currentSide,foodType,spliceFood,vlist,getNewElistData} =homeModel();
			return{
				show,
				saveSide,
				currentSide,
