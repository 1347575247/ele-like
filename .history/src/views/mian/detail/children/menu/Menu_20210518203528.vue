<template>
  <div class="menu">
    ...
    <div class="menu_main flex" :style="{height: '17.710526rem'}" ref="menu_main" v-if="menu">
      <ul class="main_class">
        <li class="class_item" v-for="(item,i) in menu" :key="i" @click="menuSelect(i)" :style="{background:(i===menu_select?'#ccc':'#fff')}">
          <p>
...
<script>
  ...
  export default {
    components: {},
    setup(props, ctx) {
      const {menu, recommend, menuSelect, menu_index} = menuModel()
      console.log(menu)
      return {
        menu,
        recommend,
