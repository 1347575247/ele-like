<template>
  <div class="menu">
        ...
        <ul class="view_slide flex">
          <li class="slide_item" v-for="(item, i) in recommend[0].items" :key="i" >
            ...
            <div class="item_count flex">
              ...
              <BaseButton :count="count(item)" @changeNum="changeNum(asignOther(item), $event)"/>
            </div>
    ...
    <div class="menu_main flex" :style="{height: con_height}" ref="menu_main" v-if="menu">
      <ul class="main_class">
        <li class="class_item" v-for="(item,i) in menu" :key="i" @click="menuSelect(i)" :style="{background:(i===menu_index?'#ccc':'#fff')}">
          <p>
      ...
      <div class="main_con grow_shrink">
        <div class="slideWrap">
          <dl class="con_item" v-for="(item,ci) in menu" :key="ci" >
          <!-- 分类标题 -->
          <dt class="item_title">
          ...
          <dd class="item_desc flex" v-for="(food,j) in item.foods" :key="j">
            ...
            <div class="desc_main">
              ...
              <div class="main_btn flex">
                ...
                <BaseButton :count="count(food)" @changeNum="changeNum(asignOther(food), $event)"/>
              </div>
...
<script>
  ...
  import heightModel from './js/heightModel'
  ...
  export default {
    ...
    setup(props, ctx) {
      const {menu, recommend, count, changeNum, asignOther} = menuModel()
      const {menuSelect, menu_index, con_height} = heightModel()
