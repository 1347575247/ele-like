        ...
        <ul class="view_slide flex">
          <li class="slide_item" v-for="(item, i) in recommend[0].items" :key="i" >
            ...
            <div class="item_count flex">
              ...
              <BaseButton :count="count" @changeNum="changeNum(item, $event)"/>
            </div>
          ...
          <dd class="item_desc flex" v-for="(food,j) in item.foods" :key="j">
            ...
            <div class="desc_main">
              ...
              <div class="main_btn flex">
                <div class="btn_price">
                  <span class="price">{{food.specfoods[0].original_price}}</span>
                  <span class="originPrice" v-if="food.specfoods[0].original_price">{{food.specfoods[0].original_price}}</span>
                </div>
                <BaseButton :count="count" @changeNum="changeNum(food, $event)"/>
              </div>
...
<script>
  ...
  export default {
    ...
    setup(props, ctx) {
      const {menu, recommend, menuSelect, menu_index, con_height, count, changeNum} = menuModel()
      console.log(menu)
      return {
        menu,
        recommend,
