<template>
  <div class="menu">
    <!-- 推荐 -->
    <div class="menu_recommend" v-for="(item, i) in recommend[0].items" :key="i">
      <h3 class="recommend_title">商家推荐</h3>
      <div class="recommend_view">
        <ul class="view_slide flex">
          <li class="slide_item">
            <img class="item_img" :src="1" />
...
<script>
  import monuModel from './js/menuModel'
  export default {
    components: {},
    setup(props, ctx) {
      const {menu, recommend} = monuModel()
      console.log(recommend.value[0].items)
      return {
        menu,
        recommend
      }
